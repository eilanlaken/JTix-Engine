...
public class Queue<T> implements Iterable<T> {
    ...
    public T removeFirst () {
        if (size == 0) {
            // Underflow
            throw new IllegalStateException("Trying to remove from empty " + this.getClass().getSimpleName());
        }
    ...
    public T last () {
        if (size == 0) {
            // Underflow
            throw new IllegalStateException("Trying to get last element of an empty " + this.getClass().getSimpleName());
        }
        final T[] values = this.values;
        int tail = this.tail;
        tail--;
...
// QueueIterable<T> iterable;
        ...
        public T next () {
            if (index >= queue.size) throw new IllegalStateException(String.valueOf(index));
            if (!valid) {
                throw new IllegalStateException("#iterator() cannot be used nested.");
            }
