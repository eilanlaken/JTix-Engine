...
import com.heavybox.jtix.shape.ShapeUtils;
...
public final class BodyColliderPolygon extends BodyCollider {
    ...
    public BodyColliderPolygon(float density, float staticFriction, float dynamicFriction, float restitution, boolean ghost, int bitmask,
        ...
        if (polygon.length < 6) throw new IllegalArgumentException("At least 3 points are needed to construct a polygon; Points array must contain at least 6 values: [x0,y0,x1,y1,x2,y2,...]. Given: " + polygon.length);
        if (polygon.length % 2 != 0) throw new IllegalArgumentException("Point array must be of even length in the format [x0,y0, x1,y1, ...].");
        this.vertexCount = polygon.length / 2;
        int windingOrder = MathUtils.polygonWindingOrder(polygon);
        this.vertices = MemoryUtils.copy(polygon);
