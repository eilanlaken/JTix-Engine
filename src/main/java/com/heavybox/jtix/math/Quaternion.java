...
//    }
    ...
    public Quaternion setFromAxisRad(final float x, final float y, final float z, final float radians) {
        ...
        if (d == 0f) return idt();
        d = 1f / d;
        float l_ang = radians < 0 ? MathUtils.PI_TWO - (-radians % MathUtils.PI_TWO) : radians % MathUtils.PI_TWO;
    ...
    public Quaternion setFromMatrix(boolean normalizeAxes, Matrix4x4 matrix) {
        return setFromAxes(normalizeAxes, matrix.val[Matrix4x4.M00], matrix.val[Matrix4x4.M01], matrix.val[Matrix4x4.M02],
                matrix.val[Matrix4x4.M10], matrix.val[Matrix4x4.M11], matrix.val[Matrix4x4.M12], matrix.val[Matrix4x4.M20],
                matrix.val[Matrix4x4.M21], matrix.val[Matrix4x4.M22]);
    }
