...
public class Renderer2D_3 implements MemoryResourceHolder {
    ...
    public void drawPolygonThin(float[] polygon, boolean triangulated, float x, float y, float degrees, float scaleX, float scaleY) {
        ...
        if (!triangulated) {
            ...
            ArrayFloat verticesPolygon = arrayFloatPool.allocate();
            ArrayInt   indicesPolygon  = arrayIntPool.allocate();
            ...
            try {
                MathUtils.polygonTriangulate(polygon, verticesPolygon, indicesPolygon);
            } catch (Exception e) {
            ...
            Vector2 vertex = vectors2Pool.allocate();
            for (int i = 0; i < verticesPolygon.size; i += 2) {
                float poly_x = verticesPolygon.get(i);
                float poly_y = verticesPolygon.get(i + 1);
            ...
            for (int i = 0; i < indicesPolygon.size - 2; i += 3) {
                indices.put(startVertex + indicesPolygon.get(i));
                indices.put(startVertex + indicesPolygon.get(i + 1));

                indices.put(startVertex + indicesPolygon.get(i + 1));
                indices.put(startVertex + indicesPolygon.get(i + 2));

                indices.put(startVertex + indicesPolygon.get(i + 2));
                indices.put(startVertex + indicesPolygon.get(i));
            }

            vertexIndex += verticesPolygon.size / 2;

            arrayFloatPool.free(verticesPolygon);
            arrayIntPool.free(indicesPolygon);
        }
    }
