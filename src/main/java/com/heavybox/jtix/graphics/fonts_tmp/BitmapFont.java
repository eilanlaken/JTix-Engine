package com.heavybox.jtix.graphics.fonts_tmp;
...
public class BitmapFont {
    ...
    public BitmapFont (BitmapFontData data, TextureRegion region, boolean integer) {
        this(data, region != null ? Array.of(region) : null, integer);
    }
    ...
    // Drawing operations - will be implemented after basic rendering 2d API is complete.
    ...
    static public class BitmapFontData {
        ...
        public void load (FileHandle fontFile, boolean flip) {
            ...
            try {
                ...
                if (line == null) throw new GraphicsException("File is empty.");

                line = line.substring(line.indexOf("padding=") + 8);
                String[] padding = line.substring(0, line.indexOf(' ')).split(",", 4);
                if (padding.length != 4) throw new GraphicsException("Invalid padding.");
                ...
                if (line == null) throw new GraphicsException("Missing common header.");
                String[] common = line.split(" ", 9); // At most we want the 6th element; i.e. "page=N"

                // At least lineHeight and base are required.
                if (common.length < 3) throw new GraphicsException("Invalid common header.");

                if (!common[1].startsWith("lineHeight=")) throw new GraphicsException("Missing: lineHeight");
                lineHeight = Integer.parseInt(common[1].substring(11));

                if (!common[2].startsWith("base=")) throw new GraphicsException("Missing: base");
                ...
                for (int p = 0; p < pageCount; p++) {
                    ...
                    line = reader.readLine();
                    if (line == null) throw new GraphicsException("Missing additional page definitions.");
                    ...
                    if (matcher.find()) {
                        ...
                        try {
                            ...
                            if (pageID != p) throw new GraphicsException("Page IDs must be indices starting at 0: " + id);
                        } catch (NumberFormatException ex) {
                            throw new GraphicsException("Invalid page id: " + id + ". Exception: " + ex.getMessage());
                        }
                    }

                    matcher = Pattern.compile(".*file=\"?([^\"]+)\"?").matcher(line);
                    if (!matcher.find()) throw new GraphicsException("Missing: file");
                ...
                throw new GraphicsException("Error loading font file: " + fontFile + ". Exception: " + ex.getMessage());
            } finally {
                try {
                    reader.close();
        ...
        public Glyph getFirstGlyph () {
            ...
            throw new GraphicsException("No glyphs found.");
        }
        ...
        public void getGlyphs (GlyphRun run, CharSequence str, int start, int end, Glyph lastGlyph) {
            ...
            ArrayFloat xAdvances = run.xAdvances;
