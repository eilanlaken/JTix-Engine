...
public class GlyphLayout implements MemoryPool.Reset {

    private static final MemoryPool<GlyphRun> glyphRunPool = new MemoryPool<>(GlyphRun.class, 10);
    private static final ArrayInt colorStack = new ArrayInt(4);
    ...
    public final ArrayInt colors = new ArrayInt(2);
     ...
     *           {@link BitmapFont.BitmapFontData#markupEnabled} is true, color markup tags in the specified string may change the color for
    ...
    public void setText (BitmapFont font, CharSequence str, int start, int end, Color color, float targetWidth, int halign,
        ...
        while (true) {
            ...
            runEnded:
            {
                ...
                GlyphRun run = glyphRunPool.allocate();
                run.x = 0;
                run.y = y;
    ...
    private void truncate (BitmapFont.BitmapFontData fontData, GlyphRun run, float targetWidth, String truncate) {
        ...
        GlyphRun truncateRun = glyphRunPool.allocate();
    ...
    private GlyphRun wrap (BitmapFont.BitmapFontData fontData, GlyphRun first, int wrapIndex) {
        ...
        ArrayFloat xAdvances2 = first.xAdvances; // Starts with all the xadvances.
        ...
        if (secondStart < glyphCount) {
            ...
            ArrayFloat xAdvances1 = second.xAdvances; // Starts empty.
    ...
    private float getLineOffset (Array<Glyph> glyphs, BitmapFont.BitmapFontData fontData) {
    ...
    private int parseColorMarkup (CharSequence str, int start, int end) {
        ...
        for (int i = start + 1; i < end; i++) {
            char ch = str.charAt(i);
            if (ch != ']') continue;
            Color color = Color.getFromCatalog(str.subSequence(start, i).toString());
