...
public class BitmapFontCache {
    ...
    private final MemoryPool<GlyphLayout> glyphLayoutMemoryPool = new MemoryPool<>(GlyphLayout.class, 2);
    ...
    private ArrayInt[] pageGlyphIndices;
    ...
    public BitmapFontCache (BitmapFont font, boolean integer) {
        ...
        if (pageCount > 1) {
            ...
            pageGlyphIndices = new ArrayInt[pageCount];
            for (int i = 0, n = pageGlyphIndices.length; i < n; i++)
                pageGlyphIndices[i] = new ArrayInt();
        }
        ...
        for (int i = 0, n = layouts.size; i < n; i++) {
            ...
            for (int ii = 0, nn = layout.runs.size; ii < nn; ii++) {
                ...
                for (int iii = 0, nnn = run.glyphs.size; iii < nnn; iii++) {
                    ...
                    int page = ((Glyph)glyphs[iii]).page;
    ...
    public void setColors (float color, int start, int end) {
        ...
        for (int i = 0; i < pageCount; i++) {
            float[] vertices = pageVertices[i];
            ArrayInt glyphIndices = pageGlyphIndices[i];
    ...
    public void clear () {
        x = 0;
        y = 0;
        glyphLayoutMemoryPool.freeAll(pooledLayouts);
        pooledLayouts.clear();
        layouts.clear();
    ...
    private void setPageCount (int pageCount) {
        ...
        ArrayInt[] newPageGlyphIndices = new ArrayInt[pageCount];
        ...
        for (int i = pageGlyphIndicesLength; i < pageCount; i++)
            newPageGlyphIndices[i] = new ArrayInt();
    ...
    private void addToCache (GlyphLayout layout, float x, float y) {
        ...
        for (int i = 0; i < runCount; i++) {
            ...
            for (int ii = 0, nn = run.glyphs.size; ii < nn; ii++) {
                if (glyphIndex++ == nextColorGlyphIndex) {
                    lastColorFloatBits = Color.intToFloatColor(colors.get(++colorsIndex));
                    nextColorGlyphIndex = ++colorsIndex < colors.size ? colors.get(colorsIndex) : -1;
                }
                gx += xAdvances[ii];
     ...
     * @param y The y position for the top of most capital letters in the font (the {@link BitmapFont.BitmapFontData#capHeight cap height}).
    ...
    public GlyphLayout addText (CharSequence str, float x, float y, int start, int end, float targetWidth, int halign,
                                boolean wrap, String truncate) {
        GlyphLayout layout = glyphLayoutMemoryPool.allocate();
