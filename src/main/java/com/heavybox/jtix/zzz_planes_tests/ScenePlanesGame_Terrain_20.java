...
public class ScenePlanesGame_Terrain_20 implements Scene {
    ...
    public ScenePlanesGame_Terrain_20() {
    }

    @Override
    public void setup() {
        Assets.loadModel("assets/game-models/tree_1_LOD_1.fbx", "assets/game-textures");
        Assets.finishLoading();

        treeModel = Assets.get("assets/game-models/tree_1_LOD_1.fbx");
...
//        System.out.println(camera.position.z);
        ...
        camera.update();
        if (Input.keyboard.isKeyPressed(Keyboard.Key.A)) height += delta * 100;
        if (Input.keyboard.isKeyPressed(Keyboard.Key.Z)) height -= delta * 100;
        update_gameplay();
        ...
        // draw pulled objects, maybe after terrain
        // this will be per camera attribute
        Renderer3D.begin(camera);
        GL11.glEnable(GL11.GL_POLYGON_OFFSET_FILL);
        GL11.glPolygonOffset(-1.0f, -1.0f); // pulls stuff towards the camera
//        for (int i = 0; i < trees.model.materials.length; i++) {
//            Renderer3D.drawMesh(Renderer3D.defaultShaderPBR, trees.model.meshes[i], trees.model.materials[i], new Matrix4x4().translateGlobalAxisXYZ(0,0,height));
//        }
        GL11.glDisable(GL11.GL_POLYGON_OFFSET_FILL);
        Renderer3D.end();

        Renderer3D.begin(camera);
        Renderer3D.drawModel(treeModel, new Matrix4x4().translateGlobalAxisXYZ(0,-200,height));
        for (int i = 0; i < 64; i++) {
            for (int j = 0; j < 64; j++) {
