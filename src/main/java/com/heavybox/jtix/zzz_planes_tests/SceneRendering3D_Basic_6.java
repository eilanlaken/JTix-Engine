...
public class SceneRendering3D_Basic_6 implements Scene {
    ...
    public Vector3 CAMERA_OFFSET_DEFAULT = new Vector3(0, 0, 1.6f);
    public Vector3 COCKPIT_LOOK_AT_TARGET = new Vector3(0, 1.78f, 1.05f);

    public SceneRendering3D_Basic_6() {
    }
    ...
    private void update_cockpit() {
        Vector3 modelForward = new Vector3();
        cockpit.transform.getBasisY(modelForward);

        if (Input.keyboard.isKeyPressed(Keyboard.Key.LEFT)) {
            cockpit.transform.rotateLocalAxisY(1);

            Vector3 lookAt = cockpit.transform.getTranslation(new Vector3());
            Vector3 lookAtOffset = new Vector3(COCKPIT_LOOK_AT_TARGET).rotate(modelForward, 1);
            lookAt.add(lookAtOffset);

            //camera.lookAt(cockpit.transform.getPositionX(), cockpit.transform.getPositionY(), cockpit.transform.getPositionZ());
        ...
        if (Input.keyboard.isKeyPressed(Keyboard.Key.RIGHT)) {
            ...
            camera.lookAt(cockpit.transform.getTranslationX(), cockpit.transform.getTranslationY(), cockpit.transform.getTranslationZ());
            camera.rotateAroundForward(-1);


        }

        camera.position.set(CAMERA_OFFSET_DEFAULT)
                .add(cockpit.transform.getTranslationX(), cockpit.transform.getTranslationY(), cockpit.transform.getTranslationZ());

        float delta = Graphics.getDeltaTime();
        cockpit.transform.translateGlobalAxisXYZ(modelForward.x * delta, modelForward.y * delta, modelForward.z * delta);
