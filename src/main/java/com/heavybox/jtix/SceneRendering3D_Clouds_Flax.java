...
// This is an implementation from this Flax engine tutorial.
// https://www.youtube.com/watch?v=jUGln7TE1Gk&t=2s
public class SceneRendering3D_Clouds_Flax implements Scene {
    ...
    public SceneRendering3D_Clouds_Flax() {
    }
    ...
    public void setup() {
        ...
        Assets.loadModel("assets/app-models/cloud-simplification.fbx", "assets/app-models/textures");
        ...
        // setup clouds
        modelCloud = Assets.get("assets/app-models/cloud-simplification.fbx");
        modelCockpit = Assets.get("assets/app-models/cockpit-template.fbx");
        cloudAtlas = Assets.get("assets/app-textures/cloud-atlas.png");
        cloudFade = Assets.get("assets/app-textures/cloud-fade.png");
        ...
        modelCloud.materials[0].materialAttributes.put("u_texture_fade", cloudFade);
        String vertexShaderSrc = Assets.getFileContent("assets/app-shaders/cloud-shader-meh.vert");
        String fragmentShaderSrc = Assets.getFileContent("assets/app-shaders/cloud-shader-meh.frag");
    ...
    public void start() {
        ...
        cockpit = new Entity();
        cockpit.transform = new Matrix4x4().translateGlobalAxisXYZ(0, 0, 0);
        cockpit.model = modelCockpit;
        entities.add(cockpit);

        final int CLOUDS_COUNT = 1;
        final float range = 0.26f * CLOUDS_COUNT;
        final float scale = 10.4f * (CLOUDS_COUNT / range);

        for (int i = 0; i < CLOUDS_COUNT; i++) {
            Entity entity = new Entity();
            entity.transform = new Matrix4x4().translateGlobalAxisXYZ(MathUtils.randomUniformFloat(-range,range), MathUtils.randomUniformFloat(-range,range), 15 + MathUtils.randomUniformFloat(-range/4,range/4));
    ...
    public void update() {
        ...
        ShaderBinder.bind(cloudShader);
        cloudShader.bindUniform("u_camera_forward", camera.forward);
    ...
    @Deprecated private void orient2(Matrix4x4 transform) {
        Vector3 position = transform.getPosition(new Vector3());
        Vector3 target_orientation = new Vector3(camera.position).sub(position).nor(); // this will be the 3rd column
        Vector3 right = new Vector3(target_orientation).crs(camera.up);
        //Vector3 right = new Vector3(camera.right);
        Vector3 up = new Vector3(target_orientation).crs(right).nor();
