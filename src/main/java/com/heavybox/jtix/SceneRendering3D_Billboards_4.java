...
public class SceneRendering3D_Billboards_4 implements Scene {
    ...
    public void update() {
        ...
        if (Input.keyboard.isKeyJustPressed(Keyboard.Key.M)) {
            //orient_4();
            pure_billboard(transformCloud_1);
        }
    }
    ...
    private void pure_billboard(Matrix4x4 transform) {
        Vector3 position = transform.getPosition(new Vector3());

        // rotate the entire gizmo
        Vector3 basisY = new Vector3(camera.forward).negate();
        Vector3 basisZ = new Vector3(camera.up); // camera up
        Vector3 basisX = new Vector3(basisZ).crs(basisY);
        transform.setFromBasis(basisX, basisY, basisZ, position);
        System.out.println(Vector3.areOrthonormal(basisX, basisY, basisZ));
    }
