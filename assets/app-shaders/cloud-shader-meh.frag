...
in float alpha_dir_factor;

// uniforms - PBR material
uniform sampler2D u_texture_atlas;
uniform sampler2D u_texture_fade;
...
void main()
{
    ...
    vec4 color_cloud = texture(u_texture_atlas, newUV);
    vec4 color_fade = texture(u_texture_fade, uv);
    float opacity = fade * min(color_cloud.a, color_fade.a) ;
    //color = texture(u_texture_fade, uv);
    //color.a *= fade * 0.8;
    out_color = vec4(color_cloud.rgb, opacity);
    //out_color = vec4(nor, 1);
}